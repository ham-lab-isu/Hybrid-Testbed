...
class Kaw2FFFControl:
    def __init__(self, root):
        ...
        print("server_thread has been started")
    ...
    def process_received_data(self, data):
        ...
        if "channel_1_analog:" in data_str:
            # Example: channel_1_analog:3.14,2.56,1.23,0.45,5.00,4.89,3.33,2.22
            analog_values = data_str.split("channel_1_analog:")[1].split(",")
            for pin, value in enumerate(analog_values, start=1):
                self.update_channel_1_analog_value(pin, float(value))
    ...
    def initialize_gui(self):
        ...
        self.create_io_controls(self.IO_control_tab, 1, 15, 0)
        self.create_io_controls(self.IO_control_tab, 2, 7, 1)
        self.create_io_controls(self.IO_control_tab, 3, 15, 2)
        self.create_io_controls(self.IO_control_tab, 4, 15, 3)

         # Adding Channel 1 Analog Input Signals Frame
        self.channel_1_analog_frame = ttk.LabelFrame(self.IO_control_tab, text="Channel 1 Analog Inputs", padding="10")
        self.channel_1_analog_frame.grid(row=1, column=0, padx=5, pady=5, sticky=(tk.W, tk.E))

        # Create labels for Channel 1 analog input pins
        self.channel_1_analog_labels = {}
        for pin in range(1, 9):  # Assuming 8 pins for analog inputs
            label = ttk.Label(self.channel_1_analog_frame, text=f"Pin {pin}: 0.00 V", width=25, anchor="w")
            label.grid(row=(pin - 1) // 4, column=(pin - 1) % 4, padx=5, pady=5, sticky="w")
            self.channel_1_analog_labels[pin] = label
    ...
    def update_channel_1_analog_value(self, pin, value):
        #Update the displayed value for a given analog input pin.
        if pin in self.channel_1_analog_labels:
            label = self.channel_1_analog_labels[pin]
            label.config(text=f"Pin {pin}: {value/1000:.2f} V")
